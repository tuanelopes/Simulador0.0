...
!
    ...
    !! @param var_n         Valor n.
    subroutine readOutFlagKeyword(keyword, target, var_out, var_n, ierr)
        implicit none
        character(50) keyword
...
!        write(*,'(a)') " em subroutine leituraGeracaoConectividadesDS(keyword_name, conecElem, mat, nen)"
    ...
    subroutine readNodeElementsDS
        ...
        keyword_name = "ntype"
        call readIntegerKeywordValue(keyword_name, ntype, ntype, ierr)
        keyword_name = "numat"
        call readIntegerKeywordValue(keyword_name, numat, numat, ierr)
        keyword_name = "nen"
        call readIntegerKeywordValue(keyword_name, nen, nen, ierr)
        keyword_name = "nicode"
        call readIntegerKeywordValue(keyword_name, nicode, nicode, ierr)
    end subroutine !***************************************************************************************************


    !> Efetua a leitura de propriedades de materiais.
    !> @param keyword_name  Keyword especifica das  propriedades de materiais.
    subroutine readMaterialPropertiesDS(keyword_name, ierr)
        use mGlobaisEscalares
